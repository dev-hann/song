...
- 허용 속도: 0.5x, 0.75x, 1x, 1.25x, 1.5x, 2x (사용자 정의 불가)
- 기본값: 1x
- 속도 변경 시 MediaSession position state 동기화
...
| 탭 | 설명 |
|----|------|
| 재생 (controls) | 앨범 아트, 재생 컨트롤, 시크바, 속도/반복/셔플/자동재생 토글 |
| 가사 (lyrics) | 시간 동기화 가사. 현재 줄 자동 스크롤, 줄 탭 시 seek (가사.md 참조) |
| 추천 곡 (related) | 현재 트랙 기반 관련 영상 목록. 탭 시 recommendedQueue로 로드 |
...
| Action | 검색 | 홈(차트) | 홈(추천) | 좋아요 | 최근재생 | 재생목록 |
|--------|:----:|:-------:|:-------:|:----:|:------:|:------:|
| 바로 재생 | ✓ | ✓ | ✓ | ✓ | ✓ | ✓ |
| 큐에 추가 | ✓ | ✓ | ✓ | ✓ | ✓ | ✓ |
| 다음에 재생 | ✓ | ✓ | ✓ | ✓ | ✓ | ✓ |
...
재생 시스템은 브라우저 전역에 영향을 미치는 부수 효과가 있음:

| 효과 | 설명 |
|------|------|
| **document.title 변경** | 재생 중: `"재생 중: {제목} - SONG"`, 미재생: `"SONG - 광고 없는 영상 플레이어"` |
| **자동재생 언락** | 첫 사용자 인터랙션(click/touch/keydown) 시 무음 WAV 재생으로 브라우저 자동재생 제한 해제 |
| **visibility resume** | 탭이 다시 활성화될 때 브라우저가 일시정지한 오디오를 자동 복구 |
