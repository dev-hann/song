...
function Slider({
  ...
  const handleTrackClick = useCallback(
    (e: React.MouseEvent<HTMLDivElement>) => {
      ...
      const newValue = min + percent * ((max as number) - (min as number))
      onValueChange(newValue as never, {} as never)
    },
      ...
      >
        ...
        {Array.from({ length: _values.length }, (_, index) => (
          <SliderPrimitive.Thumb
            ...
            key={index}
            className="relative block size-3.5 shrink-0 rounded-full border border-primary bg-primary ring-ring/50 transition-[color,box-shadow] select-none after:absolute after:-inset-2 hover:ring-3 focus-visible:ring-3 focus-visible:outline-hidden active:ring-3 disabled:pointer-events-none disabled:opacity-50"
          />
        ))}
      </SliderPrimitive.Control>
    </SliderPrimitive.Root>
