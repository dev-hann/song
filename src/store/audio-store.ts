...
import { fetchAudioInfo, fetchRelatedTracks } from '@/services/api';
...
export const useAudioStore = create<AudioStore>((set, get) => ({
  ...
  setAudio: (audio: Audio) => {
    ...
    audioPlayer.load(audio.id, audio).catch(() => undefined);
  },
  ...
  toggleLike: () => {
    ...
    (async () => {
      try {
        ...
        if (newLiked) {
          const { addLike } = await import('@/services/api');
          await addLike(track);
        } else {
          const { removeLike } = await import('@/services/api');
  ...
  playPrevious: () => {
    ...
    audioPlayer.load(prev.id, prev).catch(() => undefined);
  },
  ...
  setQueue: (tracks, startIndex = 0) => {
    ...
    audioPlayer.load(track.id, track).catch(() => undefined);
  },
